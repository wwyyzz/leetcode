package Question.SlidingWindowAndTwoPointers.Q27_pass;

import org.junit.Test;

public class Solution1Test {

    @Test
    public void removeElement() {
//        assertEquals(2, new Solution1().removeElement(new int[]{3,2,2,3},3));
//        assertEquals(5, new Solution1().removeElement(new int[]{0,1,2,2,3,0,4,2},2));
        int[] result ={2,2};

//        assertArrayEquals(result, new Solution2().removeElement(new int[]{3,2,2,3},3));
//        assertArrayEqual(result, new Solution2().removeElement(new int[]{3,2,2,3},3));
//        assertEquals(5, new Solution2().removeElement(new int[]{0,1,2,2,3,0,4,2},2));

    }
}